...
1. Agent resolution uses checkpoint `"plan"` -- `node ~/.claude/get-shit-done/bin/gsd-tools.cjs coplanner agents "plan"`
2. The artifact(s) read from disk are ALL PLAN.md files in the phase directory: `cat "${PHASE_DIR}"/*-PLAN.md`
3. Also read REQUIREMENTS.md and ROADMAP.md for context (plans implement requirements per roadmap phases)
...
6. Synthesis applies accepted changes to the relevant PLAN.md file(s) via Edit tool
7. The conditional commit message is `docs: incorporate co-planner feedback (plan)`
...
1. Agent resolution uses checkpoint `"verification"` -- `node ~/.claude/get-shit-done/bin/gsd-tools.cjs coplanner agents "verification"`
2. The artifact read from disk is VERIFICATION.md from the phase directory: `cat "${PHASE_DIR}"/*-VERIFICATION.md`
3. Also read the phase goal from ROADMAP.md and the must_haves from the PLAN.md frontmatter for context
...
5. Ask agent to organize response into three sections: **Suggestions**, **Challenges**, **Endorsements**
6. Synthesis applies accepted changes to VERIFICATION.md via Edit tool
7. The conditional commit message is `docs: incorporate co-planner feedback (verification)`
...
**Part B: Update skip-to reference**

One existing reference points to step 7.5 and needs updating:

Line 111: `Continue to step 7.5 (adversary review). Status routing happens after adversary review.` -- change to `Continue to step 7.3 (co-planner review). Status routing happens after adversary review.`

This ensures the verification flow routes through co-planner review before adversary review. The co-planner section handles skip logic and falls through to 7.5.

Total: ~100 lines of markdown instructions + 1 line edit.
  </action>
  <verify>
Grep for "7.3" in commands/gsd/execute-phase.md -- must find the new section. Verify "7.3" appears BEFORE "7.5" in the file. Verify the section contains: `coplanner agents "verification"`, `coplanner invoke`, `Co-Planner Synthesis`. Verify the old reference to "step 7.5" on line 111 now says "step 7.3".
  </verify>
  <done>
...
- Conditional commit with correct scope is present in both sections
</success_criteria>
